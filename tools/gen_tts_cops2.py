# -*- coding: utf-8 -*-
"""Z宇宙新增副本·第二批(worlds 展示向)本地配音素材生成（暂存版）。

覆盖 20 个新增副本(00_INDEX_EXPANSION.md §1 的 BOSS宣言/钩子),worlds 展示向台词。
仿 gen_tts_z_worlds.py 的 Qwen3-TTS CustomVoice 用法,但:
  - 不修改任何既有 .rs/.js/.json 文件。
  - manifest 独立使用 tools/assets_manifest_cops2.json(新建)。
  - 输出写 tools/design/audio_cops2/*.wav(暂存),不写 server-rs/ui/assets/audio。
  - 每条最多重试 3 次:首次用 manifest 指定 speaker;失败/时长<0.5s 则按音色族回退重试。
运行:
  PS> $env:PYTHONIOENCODING="utf-8"; & D:\\ai_vllm_env\\Scripts\\python.exe tools/gen_tts_cops2.py
"""
import json, os, time

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
OUT = os.path.join(ROOT, "tools", "design", "audio_cops2")
...
def main():
    ...
    manifest = json.load(open(os.path.join(ROOT, "tools", "assets_manifest_cops2.json"), encoding="utf-8-sig"))
    ...
    for v in manifest["voices"]:
        ...
        for i, spk in enumerate(attempts[:MAX_ATTEMPTS]):
            ...
            try:
                ...
                if dur > 0.5:
                    ok = True
                    print(f"DONE {vid} slug={v.get('slug','')} speaker={spk} attempt={done_at} dur={dur:.2f}s size={os.path.getsize(path)}", flush=True)
                    results.append({"id": vid, "slug": v.get("slug", ""), "ok": True, "speaker": spk, "attempt": done_at,
                ...
                time.sleep(1)
        if not ok:
            results.append({"id": vid, "slug": v.get("slug", ""), "ok": False, "attempt": "MAX_RETRIES", "speaker": last_speaker, "path": path,
                            "text": v["text"]})
            print("FAILED_ALL", vid, flush=True)

    # 汇总落盘
    summ = os.path.join(OUT, "_generate_summary.json")
