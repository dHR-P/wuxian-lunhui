# -*- coding: utf-8 -*-
"""qa_boss_v3.py — 用 tokenrhythm/qwen3.7-flash 按 round3 正确口径质检伽椰子立绘单张图。
用法:
  python qa_boss_v3.py <img> <tag> <outjson>
维度: object / composition / background / integrity, 输出 JSON。
口径(round3 校准): 指缝是黑发缠绕(非绳索!),故不要求绳索;发尾羽化入纯黑为设计意图,不算渐变/泛光缺陷;
  background 只查: 底部无地面/无投影/整体纯黑;cut 图透明背景为抠图正常结果,不算缺陷。
"""
...
CRITERIA = (
    ...
    "   !!! 注意:设定中没有绳索元素,不得要求画面出现绳索,也不得以'缺少绳索'判 FAIL。\n"
    "   注意:正式设定是四肢着地爬行+头颈反折回望,若是这个姿态属符合设定,不是缺陷。\n"
    ...
    "4) integrity 0-1: 指缝黑发缠绕可辨、手指未融合、无白色描边、无多余物件。\n"
...
def ask(img):
    ...
    body = json.dumps({"model": MODEL, "messages": [
        {"role": "system", "content": "你是一名严格、客观的视觉质检员(视觉模型 qwen3.7-flash)。"
                                      "严格依据给定正式设定与判据逐项判定,如实报告,不臆造,不猜测身份。"},
        {"role": "user", "content": [{"type": "text", "text": USER_TAIL},
...
def main():
    ...
    result = {"tag": tag, "file": os.path.basename(img), "path": img,
